...
public class ExpandTabView extends LinearLayout implements OnDismissListener {
	...
	public void setPhoto(int valuePhoto, int position) {
		if (position < mToggleButton.size()) {
			...
			mToggleButton.get(position).setCompoundDrawablesWithIntrinsicBounds(valuePhoto,0, R.drawable.ic_routelist_dropdown_arrow,0);
		}
	}
	...
	public void setValue(ArrayList<String> textArray, ArrayList<View> viewArray,int[] photo) {
		...
		for (int i = 0; i < viewArray.size();i++) {
			...
			tButton.setCompoundDrawablesWithIntrinsicBounds(photo[i],0,R.drawable.ic_routelist_dropdown_arrow,0);
			addView(tButton);
